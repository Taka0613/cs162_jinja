...
@app.route("/task_groups/new", methods=["GET", "POST"])
@login_required
...
def add_task(group_id):
    task_group = TaskGroup.query.get_or_404(group_id)
    if task_group.user_id != current_user.id:
        flash("You do not have permission to add tasks to this group.")
        return redirect(url_for("task_groups"))

    if request.method == "POST":
        title = request.form["title"]
        new_task = Task(title=title, task_group_id=group_id)
